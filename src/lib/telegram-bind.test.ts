...
// 行為測試走記憶體 Map（demo 分支）。若環境帶了 Supabase 金鑰（非 demo），略過以免打到真實 app_state、結果不穩定。
const demoOnly = { skip: isDemoMode ? false : "需 demo 模式（無 Supabase 金鑰）才執行，避免外部依賴" };
...
test("createBindToken 產出合法 token，consumeBindToken 一次性消費（防重放）", demoOnly, async () => {
...
test("consumeBindToken 對格式錯誤的 token 直接回 null（不查庫）", demoOnly, async () => {
  assert.equal(await consumeBindToken("not-a-valid-token"), null);
  assert.equal(await consumeBindToken(""), null);
});

test("cleanupExpiredBindTokens 不刪除未過期的有效綁定碼", demoOnly, async () => {
