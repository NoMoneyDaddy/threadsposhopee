...
// 把生成過程拋出的錯誤對應成「對使用者有用、又不洩漏上游細節」的提示。
// 只看錯誤訊息的關鍵字分類（不回傳原始上游 body，body 可能含金鑰片段／內部資訊）。
// 找不到對應時回 fallback（由呼叫端帶入，如「生成失敗，請稍後再試」）。
export function geminiErrorMessage(e: unknown, fallback: string): string {
  ...
  if (/timeout|aborted|TimeoutError|逾時/i.test(m)) return "Gemini 回應逾時，請稍後再試";
  return fallback;
}
