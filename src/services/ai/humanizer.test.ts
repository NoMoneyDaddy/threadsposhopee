...
import { splitCopy, buildCopyPrompt, buildCopyPromptPreview, pickReplyLeadIn, HUMANIZER_RULES, ANTI_AI_SLOP_RULES } from "./humanizer";
...
test("buildCopyPrompt：硬性禁用 emoji，且 prompt 自身不含任何 emoji（含留言開場句）", () => {
  const p = buildCopyPrompt({ productName: "藍牙耳機", shopeeShortLink: "https://go2read.link/r/abc" });
  assert.match(p, /不要使用任何 emoji/);
  assert.ok(!EMOJI_RE.test(p), "prompt 內不應出現任何 emoji");
});

test("pickReplyLeadIn：穩定（同連結同結果）、無 emoji、不同連結會分散", () => {
