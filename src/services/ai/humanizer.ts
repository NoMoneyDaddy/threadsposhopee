...
// 參考並融合（並把 GitHub humanizer skill 的高訊號 pattern 收進來）：
//   - blader/humanizer（SKILL.md，本類別始祖，源自 Wikipedia「Signs of AI writing」）：除既有的有觀點/節奏/具體細節外，
//     新收進「假範圍硬湊（從X到Y）」「製造假張力的戲劇性短句連發與對話腔開場」「整段都這調調才算（別孤立殺一個詞）」三點。
...
// 取向（依 prompt 最佳實踐）：負面清單改當「要避開的調性範例」而非逐字黑名單，並用「整段都這調調才算」收尾，避免過度約束把真人開頭也壓掉。
...
- 句長刻意有落差：短句（10 字內）和長句（20 字以上）交錯，別連三句一樣長（句長有起伏是讀起來像真人最關鍵的一點）。
- 口語節奏，可以有不完整的句子、岔題、半句感想；允許一點不完美，太工整反而假。
- 第一人稱，動詞就用白話的「是／有／會」，不要為了文謅謅改成「作為／扮演著／標誌著／堪稱」。

【下面是 AI 腔的樣子，寫到就換成具體、口語的講法（這是要避開的「調性範例」，不是逐字黑名單）】
...
- 對仗／升華句型：「不只…更…」「不是…而是…」「與其說…不如說…」，以及三段式排比（硬把點子湊成三項）。
...
- 假範圍硬湊：「從…到…」把不相干的兩端湊成一句；同義詞換來換去只為了不重複（重複用同一個詞反而自然）。
- 製造假張力：別連發戲劇性短句（「老實說？」「重點來了。」），也別用「老實說」「你聽我說」這種對話腔硬開場，有話直接講。
...
判斷：孤立一兩個詞不算 AI 腔，是「整段都這調調」才要避開，別為了閃這些詞把話講得卡卡的，自然口語永遠優先。`;
