...
// 定調（誠實看待，參考 harshaneel/humanize 的限制說明）：這套靜態規則的目標是「讀起來像真人、好讀、想互動」，
// 不是規避偵測。靜態 prompt 規則可降低明顯 AI 腔，但打不過 GPTZero／Pangram 這類學習型偵測器（它們抓的是模型權重裡的 RLHF 指紋）。
// 所以我們不為了騙偵測器硬湊，自然真實永遠優先。
...
- 格言公式：別用「X 是 Y 的 Z」「X 變成一種陷阱」這種裝深刻的句式，直接講那件具體的事就好。
- 被動／無主詞碎句：「無需設定」「值得被看見」這種沒主詞的官腔，補回主詞改成「你不用設定」「我覺得很值得」。
- diff 式書寫：別描述「升級了什麼／改了哪裡」，直接講它現在用起來是怎樣。
- 繞圈填充詞：「基於…的考量」「為了能夠…」「在當前這個時間點」改成「因為」「為了」「現在」。
