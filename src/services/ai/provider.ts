...
export async function generateThreadCopy(
  ...
  const midSegRule = auto
    ? `- 主文之後、最後引導語之前，可視內容多寡加 0～數段延伸內容（語氣比照正文、字數比照正文設定盡量貼近），各推進一個重點／心得，不要放網址\n`
    : n > 2
      ? `- 第 2～${n - 1} 段是延伸內容（語氣比照正文、字數比照正文設定盡量貼近），各推進一個重點／心得，不要放網址\n`
      : ``;
  const prompt = `${HUMANIZER_RULES}
${custom}
${segInstruction}規則：
- 繁體中文、口語、無業配味，每段可獨立成立
- 串文的鉤子只用在「段與段之間」：某段後面還有內容段時，結尾可留個鉤子（懸念、半句、「結果…」「但…」）引導往下滑。但「緊接引導語前的最後一段內容」必須把話收尾講完，不可留沒解答的問題或半句，否則讀者會以為文章被截斷沒寫完
- 第 1 段是主文（吸睛開頭、帶出情境）：只有後面還有內容段時才在結尾留鉤子，否則主文要自己把重點收尾講完。不要放任何網址。主文語氣與字數：${describeMain(prefs.main)}，請盡量貼近此字數（可略增減，但別明顯超過）
