...
test("基準中位過低（低觸及帳號隨機波動）→ 不示警，避免虛警", () => {
  const posts = [
    { publishedAt: day(0), views: 0 },
    { publishedAt: day(1), views: 1 },
    { publishedAt: day(2), views: 0 },
    { publishedAt: day(5), views: 4 }, // 基準中位 = 4 < 門檻 10
    { publishedAt: day(6), views: 4 },
    { publishedAt: day(7), views: 5 }
  ];
  const r = detectReachDrop(posts);
  assert.equal(r.hasSignal, false);
});
